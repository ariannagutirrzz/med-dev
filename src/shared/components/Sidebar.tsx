...
const Sidebar: React.FC<SidebarProps> = ({
	...
	return (
		<>
			...
			{/* Floating open button for mobile when sidebar is closed */}
			{!isSidebarOpen && (
				<button
					type="button"
					onClick={onToggleSidebar}
					className="fixed top-4 left-4 z-50 sm:hidden bg-white border border-gray-200 rounded-full w-10 h-10 flex items-center justify-center shadow-lg text-gray-600"
      ...
      `}
			>
				...
				<div className="shrink-0 p-3 sm:p-4 md:p-5 border-b border-gray-200 flex-col text-center items-center gap-2 sm:gap-4 relative">
					...
					<div
						className={`flex-1 min-w-0 ${isSidebarOpen ? "block" : "hidden"}`}
					>
